export const SolveShort = (maze, startPoint, exitPoint) => {
      ...
      nodeArr.forEach((position, index) => {
        if (index === 1) {
          const index = track.findIndex(
            (index) =>
              index[0] === position.point[0] && index[1] === position.point[1]
          );
  ...
  const shortest = (short) =>
    short.length ? short.sort((a, b) => a.length - b.length)[0] : false;
...
export const Solver = (maze, startPoint) => {
  ...
  return index !== -1
    ? [
        `True, the exit locates at ${result[index][1]}`,
        true,
        result,
        treeVision,
      ]
    : ["No", false, result, treeVision];
};
